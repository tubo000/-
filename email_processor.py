...
# ----------------------------------------------------------------------
# 💡 メイン抽出関数: Outlookからメールを取得 (COM初期化削除 + ログ削除)
# ----------------------------------------------------------------------
def get_mail_data_from_outlook_in_memory(target_folder_path: str, account_name: str, read_mode: str = "all", days_ago: int = None) -> pd.DataFrame:
    ...
    if os.path.exists(db_path):
        try:
            ...
            # print(f"警告: 既存DBのEntryID読み込み失敗: {e}。全件新規として扱います。") # ログ削除
            existing_ids_set = set()
    ...
    try:
        ...
        if filter_query_list:
            ...
            try:
                ...
                print(f"警告: Outlook Restrict失敗: {restrict_error}") # この警告は残す
                items = target_folder.Items
                
        try:
            items.Sort("[ReceivedTime]", True)
        except Exception as sort_error:
             print(f"警告: Outlook Sort失敗: {sort_error}") # この警告は残す
                ...
                try:
                    ...
                    try:
                        mail_entry_id = str(getattr(mail_item, 'EntryID', 'UNKNOWN_ID'))
                         ...
                         # print(f"  -> 警告: EntryID取得エラー: {id_err}") # ログ削除
                    ...
                    try:
                        ...
                        # print(f"  -> 警告: 件名取得エラー (ID: {mail_entry_id}): {subj_err}") # ログ削除
                        subject = "[件名取得エラー]"

                    # print(f"\n[{processed_item_count}] 処理中: {subject[:50]}... (ID: ...{mail_entry_id[-20:]})") # ログ削除
                    ...
                    try:
                        ...
                        if PROCESSED_CATEGORY_NAME in current_categories:
                         ...
                         pass # print(f"  -> 警告: カテゴリ取得エラー (ID: {mail_entry_id}): {cat_err}") # ログ削除
                    ...
                    try:
                        ...
                        received_time = received_time_check
                         ...
                         # print(f"  -> 警告: 受信日時取得エラー (ID: {mail_entry_id}): {rt_err}") # ログ削除
                         received_time = datetime.datetime.now().replace(tzinfo=None)

                    try:
                        body = str(getattr(mail_item, 'Body', ''))
                    except Exception as body_err:
                        # print(f"  -> 警告: 本文取得エラー (ID: {mail_entry_id}): {body_err}") # ログ削除
                        body = "[本文取得エラー]"
                    ...
                    if skip_reason:
                        ...
                        try:
                            if mail_item and hasattr(mail_item, 'Attachments'):
                             ...
                             print(f"  -> 警告(has_files): 添付情報取得エラー: {attach_err}") # この警告は残す
                                    ...
                                    for attachment in attachments_collection:
                                        ...
                                        try:
                                            ...
                                            attachments_text += f"\n--- FILE: {attachment.FileName} ---\n{str(extracted_content)}\n"
                                             ...
                                             print(f"エラー(Attach Save/Read): COMエラー (File: {attachment.FileName}, ID: {mail_entry_id}): {com_err}") # このエラーは残す
                                             attachments_text += f"\n--- ERROR reading {attachment.FileName}: COM Error ---\n"
                                        except Exception as file_ex:
                                            print(f"エラー(Attach Save/Read): 例外 (File: {attachment.FileName}, ID: {mail_entry_id}): {file_ex}") # このエラーは残す
                                            attachments_text += f"\n--- ERROR reading {attachment.FileName}: {file_ex} ---\n"
                                        finally:
                                            if os.path.exists(temp_file_path):
                                                try: os.remove(temp_file_path)
                                                except OSError as oe: print(f"警告: 一時ファイル削除失敗: {oe}") # この警告は残す
                                 except Exception as loop_err:
                                      print(f"警告: 添付ファイルループ処理エラー (ID: {mail_entry_id}): {loop_err}") # この警告は残す
                          ...
                          if mail_item:
                              try:
                                  ...
                                  # print(f"  -> INFO: 処理済みマークを付与") # ログ削除
                                   ...
                                   print(f"  -> 警告: 抽出成功後のマーク付け失敗: {mark_e}") # この警告は残す
            
            else:
                 pass # print(f"[{processed_item_count}] スキップ: メールアイテムではありません (Class: {item.Class})") # ログ削除

            try:
                item = items.GetNext() 
            except (pythoncom.com_error, Exception) as next_err:
                 print(f"警告: GetNext() でエラー。ループ中断。エラー: {next_err}") # この警告は残す
                 break 
        ...
        if os.path.exists(temp_dir):
             ...
             except OSError as oe: print(f"警告: 一時フォルダクリーンアップ失敗: {oe}") # この警告は残す
        pass 

    # print(f"INFO: Outlookメール読み込みループ終了。処理アイテム数: {processed_item_count}, 新規抽出件数: {len(data_records)}") # ログ削除
