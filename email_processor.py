# email_processor.py (★ v3o エラー対策・ハイブリッド版 ★)
#11/13
...
def get_mail_data_from_outlook_in_memory(target_folder_path: str, account_name: str, read_mode: str = "all", days_ago: int = None, main_elements: dict = None) -> Iterator[dict]:
    ...
    try:
            ...
            if item.Class == 43:
                ...
                try:
                    ...
                    # スキップの場合、is_target は False のまま
                        ...
                        # print(f"DEBUG: Mail ID {mail_entry_id[:15]}... は '{skip_reason}' のためスキップ。") # 削除
                        pass
                        ...
                        if has_files and attachments_collection:
                            if not is_already_in_db: 
                                # print(f"DEBUG: Mail ID {mail_entry_id[:15]}... に {len(attachment_names)} 個の添付ファイルを発見。") # 削除
                                try:
                                    for attachment in attachments_collection:
                                        ...
                                        # 1. まず「高速な方法 (メモリ)」を試す
                                        try:
                                            # print(f"DEBUG:  -> '{filename}' を [高速(メモリ)] モードで試行...") # 削除
                                            ...
                                            # print(f"DEBUG:  -> '{filename}' [高速(メモリ)] 成功。 (抽出文字数: {len(extracted_content)})") # 削除
                                            ...
                                            if com_err.hresult == -2147024809:
                                                # print(f"DEBUG:  -> '{filename}' [高速(メモリ)] 失敗。 [安全(ディスク)] モードにフォールバックします。") # 削除
                                                ...
                                                try:
                                                    ...
                                                    # print(f"DEBUG:  -> '{filename}' [安全(ディスク)] 成功。 (抽出文字数: {len(extracted_content)})") # 削除
        ...
        if (data_records_batch or skip_ids_batch) and (not stop_flag or not stop_flag.is_set()):
            ...
            # print(f"INFO: 最後のバッチ (抽出:{len(data_records_batch)}件, スキップ:{len(skip_ids_batch)}件) を返します。") # 変更
            yield {
