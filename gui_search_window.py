...
class Screen1(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        # --- ▼▼▼ ここから修正 ▼▼▼ ---
        # ボタンフレーム内の列設定 (gridを使うため)
        button_frame.columnconfigure(1, weight=0) # 「戻る」ボタン用
        button_frame.columnconfigure(2, weight=0) # 「リセット」ボタン用
        button_frame.columnconfigure(3, weight=0) # 「検索」ボタン用

        # 抽出画面に戻るボタン (左から2番目) - .grid() を使用
        ttk.Button(button_frame, text="抽出画面に戻る", command=self.master.on_return_to_main).grid(row=0, column=1, padx=5, sticky='sw')

        # リセットボタン (右から2番目) - .grid() を使用
...
# --- ▼▼▼ リセット処理メソッドを追加 ▼▼▼ ---
    def reset_fields(self):
        ...
        for key in ['age', 'price', 'start']:
            if key in self.lower_widgets and self.lower_widgets[key].winfo_exists():
                 # ComboboxかEntryかでクリア方法を分ける
                 if isinstance(self.lower_widgets[key], ttk.Combobox):
                      self.lower_widgets[key].set('') # Comboboxはsetでクリア
                 else:
                      self.lower_widgets[key].delete(0, tk.END) # Entryはdeleteでクリア
