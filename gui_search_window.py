...
import re
import sqlite3 
from config import DATABASE_NAME , SKILL_LANGUAGE_PATTERNS , OS_PATTERNS , POSITION_PATTERNS
import traceback 
...
class Screen2(ttk.Frame):
    ...
    def open_email_from_entry(self):
        ...
        if hasattr(self.master, 'open_email_callback') and callable(self.master.open_email_callback):
            ...
            # print("エラー: open_email_callback が設定されていません。") # 削除
    ...
    def _debug_keyword_extraction(self, entry_id, col_name, text_content):
        ...
        if not target_keywords or not text_content:
            if not target_keywords:
                # print("🚨 警告: 参照元キーワードリストが空のため、ヒット箇所検索をスキップします。") # 削除
                pass
        ...
        # ユーザーキーワードを小文字化
            ...
            if not found:
                # print(f"DEBUG: キーワード '{user_keyword}' はどのパターン辞書にも見つかりませんでした。") # 削除
                pass
        ...
        # 正規表現を使った検索ロジック
        for skill_name, patterns in search_patterns.items():
            for pattern_str in patterns:
                try:
                    # re.IGNORECASE で大文字小文字を区別しない検索を行う
                    pattern = re.compile(pattern_str, re.IGNORECASE)
                
                    # finditerを使って全ての出現箇所を検索
                    for match in pattern.finditer(full_text):
                        ...
                        # コンテキストを取得（前後に3文字）
                        start_context = max(0, start_index - 5)
                        end_context = min(len(full_text), match.end() + 5)
                    
                        # ヒットした文字列とその周辺のコンテキストを取得
                        extracted_text = full_text[start_context:end_context].replace('\n', ' ')
                    
                        # 選択されたスキル名と、実際にマッチしたテキストを表示
            ...
            try:
                ...
                if row:
                    ...
                    if pd.notna(full_data) and str(full_data).strip() != '':
                        ...
                        # (中略) ... テキスト整形ロジック ...
                        target_keywords_fixed = r'(\s*(名\s*前|氏\s*名|所\s*属|年\s*齢|性\s*別|勤務地|単\s*価|業務\s*内容|必須\s*スキル|歓迎\s*スキル|備\s*考)\s*)'
                        ...
                        formatted_content = re.sub(
                            r'([^\n]|^)(' + target_chars + '+)', 
                            r'\1\n\2', 
                            full_text_content # ‼️ バグ修正: full_text_content -> formatted_content
                        )
    ...
    def display_search_results(self):
        ...
        if self.master.df_filtered_skills.empty or not all(col in self.master.df_filtered_skills.columns for col in self.tree['columns']):
            # print("表示するデータがないか、必要な列が不足しています。") # 削除
            return 
        for row in self.master.df_filtered_skills.itertuples(index=False):
            ...
            try:
                ...
                # print(f"🚨 Treeview挿入エラー: 行データ {values} の挿入に失敗しました: {e}") # 削除
                pass # 外部への表示は不要として、エラーを握りつぶします
