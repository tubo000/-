...
# 📌 修正: config.py から SCRIPT_DIR と OUTPUT_FILENAME を直接インポート
from config import SCRIPT_DIR, OUTPUT_FILENAME # <--- このように修正
...
def create_main_window_elements(root, setting_frame, saved_account, saved_folder, 
    ...
    # 4. Excelファイル存在チェックに基づいたボタンの有効化/無効化
    # 📌 修正1: 変数名とファイル名がEXCELを指していることを明確にする
    output_excel_path = os.path.join(SCRIPT_DIR, OUTPUT_FILENAME) # configからの定数に合わせる
    is_data_exists = False

    if os.path.exists(output_excel_path):
        try:
            # 📌 修正2: read_excelで読み込み、データ行があるかチェック
            # NOTE: 以前のコードでは try/except の外側で read_excel が使われていた
            df_check = pd.read_excel(output_excel_path)
            if not df_check.empty: 
                is_data_exists = True
        except Exception:
            # ファイル形式エラーや破損ファイルの場合は読み込み失敗とみなし、無効化
            pass 

    if not is_data_exists:
        # データが存在しない、または読み込みに失敗した場合、ボタンを無効化
        search_button.config(state=tk.DISABLED)
    else:
        # データを読み込めた場合は、ボタンを有効化
